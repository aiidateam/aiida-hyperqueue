...
def server_dir_mock_exec_command_wait(tmp_path):
    ...
    def _mock_exec_command_wait(obj, command: str, **kwargs):
        ...
        if command.startswith('hq'): # `hq` 
            command = ' '.join([f'{hq}', '--server-dir', f'{str(tmp_path.resolve())}'] + cmd_list[1:])
        elif command.startswith('nohup hq'): # `nohup hq`
            command = ' '.join([f'nohup {hq}', '--server-dir', f'{str(tmp_path.resolve())}'] + cmd_list[2:])
...
def test_server_info_direct(runner: CliRunner, aiida_computer_local, monkeypatch: pytest.MonkeyPatch, hq_env_mock_exec_command_wait, hq_env: HqEnv):
    """Test server info, test against local transport"""
    aiida_computer_local(label='localhost')

    monkeypatch.setattr(TransportClass, "exec_command_wait", hq_env_mock_exec_command_wait)
...
def test_server_info_ssh(runner: CliRunner, aiida_computer_ssh, monkeypatch: pytest.MonkeyPatch, hq_env_mock_exec_command_wait, hq_env: HqEnv):
...
def test_server_start_info_stop_circle(runner: CliRunner, aiida_computer_ssh, monkeypatch: pytest.MonkeyPatch, hq_env_mock_exec_command_wait, server_dir_mock_exec_command_wait, tmp_path):
    ...
    monkeypatch.setattr(TransportClass, "exec_command_wait", server_dir_mock_exec_command_wait)
